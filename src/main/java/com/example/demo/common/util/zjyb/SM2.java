...
public class SM2 {
	...
	public byte[] sm2GetZ(byte[] userId, ECPoint userKey) {
		...
		byte[] p = com.example.demo.common.util.zjyb.Util.byteConvert32Bytes(ecc_a);
		sm3.update(p, 0, p.length);

		p = com.example.demo.common.util.zjyb.Util.byteConvert32Bytes(ecc_b);
		sm3.update(p, 0, p.length);

		p = com.example.demo.common.util.zjyb.Util.byteConvert32Bytes(ecc_gx);
		sm3.update(p, 0, p.length);

		p = com.example.demo.common.util.zjyb.Util.byteConvert32Bytes(ecc_gy);
		sm3.update(p, 0, p.length);

		p = com.example.demo.common.util.zjyb.Util.byteConvert32Bytes(userKey.getX().toBigInteger());
		sm3.update(p, 0, p.length);

		p = com.example.demo.common.util.zjyb.Util.byteConvert32Bytes(userKey.getY().toBigInteger());
		sm3.update(p, 0, p.length);
	...
	public void sm2Sign(byte[] md, BigInteger userD, ECPoint userKey, com.example.demo.common.util.zjyb.SM2Result sm2Result) {
	...
	public void sm2Verify(byte[] md, ECPoint userKey, BigInteger r, BigInteger s, com.example.demo.common.util.zjyb.SM2Result sm2Result) {
		sm2Result.r1 = null;
		BigInteger e = new BigInteger(1, md);
		...
		if (t.equals(BigInteger.ZERO)) {
			...
			System.out.println("");
			sm2Result.r1 = e.add(x1y1.getX().toBigInteger()).mod(ecc_n);
			System.out.println("R: " + sm2Result.r1.toString(16));
			return;
		}
	}
}
