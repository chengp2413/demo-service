...
//            int p = 8 - input.length % 8;
            ...
            int p = input[index - 1];
            ret = new byte[index - p];
            System.arraycopy(input, 0, ret, 0, index - p);
        }
        return ret;
    }
    ...
    public void sm4_setkey_enc(SM4Context ctx, byte[] key) throws Exception {
        if (ctx == null) {
            throw new Exception("SM4Context is null!");
        }

        if (key == null || key.length != 16) {
            throw new Exception("SM4Key error!");
        }
    ...
    public byte[] sm4_crypt_cbc(SM4Context ctx, byte[] iv, byte[] input) throws Exception {
        if (iv == null || iv.length != 16) {
            throw new Exception("SM4 iv error!");
        }
